...
class Link
{
	int			id_		= 0;	//Идентификатор самой связи.
	static int	counter_;		//Статический счетчик для получения уникальных айди.

	char		time_date_[26] = { '\0' };	//Время и дата создания связи.

	Image*		left_ptr_		= nullptr;	//Указатель на левый объект.
	int			left_id_		= 0;		//Айди этого объекта.
	int			left_strength_	= 0;		//Сила связи для него.

	Id_string*	right_ptr_		= nullptr;	//Указатель на правый объект.
	int			right_id		= 0;
